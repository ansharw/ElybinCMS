			...
			if (in_array($ekstensi, $extensionList)){
				...
				if($image_size/1024 > 1024){
					$a = array(
						'status' => 'error',
						'title' => $lg_error,
						'isi' => $lg_imagesizetoolargemaximumis." 1MB"
					);
					echo json_encode($a);
					exit;
				}
...
/* 				if($status == 'publish'){
		...
		// jika status = draft
		if($status == 'draft'){
			$isi = lg('Berhasil disimpan ke Draf pada');
		}else{
			$isi = lg('Berhasil disimpan pada');
		}
...
/* 			$d = array(
			...
			if (in_array($ekstensi, $extensionList)){
				...
				if($image_size/1024 > 1024){
					$a = array(
						'status' => 'error',
						'title' => $lg_error,
						'isi' => $lg_imagesizetoolargemaximumis." 512KB"
					);
					echo json_encode($a);
					exit;
				}
		...
		$a = array(
			'status' => 'ok',
			'title' => $lg_success,
			'isi' => lg('Tulisan berhasil disunting.'),
			'callback' => 'edit',
			'callback_id' => $post_id,
			'callback_msg' => $callback_msg
		);
