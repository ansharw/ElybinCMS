...
?>	
								...
								<textarea name="content" cols="50" rows="20" class="form-control" id="text-editor" placeholder="<?php lg('Write your post content here...')?>"></textarea>
							  </div>
							</div> <!-- / .form-group -->
					...
					<div class="panel-body" style="box-shadow: 2px 1px 5px rgba(0,0,0,0.1); margin-bottom: 2px;">
						...
						<div id="hidden_toggle2">
							...
							<div id="aform" style="display: none">
								...
								<input id="child" type="text" class="form-control" name="post_password" style="display: none" placeholder="<?php echo lg('Write post password')?>..."/>
								<div id="ok" class="btn btn-sm btn-default"><?php echo lg('Ok')?></div>
								<a href="#" class="text-xs" id="c">&nbsp;<u><?php echo lg('Cancel')?></u></a>
							</div>
						</div>
		...
		<form class="" action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
			<div class="row">
				...
				foreach($pr as $cpr){
					...
					echo '	<div class="panel-body">';
					echo 		'<a href="?mod=post&amp;act=restore&amp;id='. $cpr->post_id.'&amp;clear=yes" class="btn btn-primary pull-right"><i class="fa fa-rotate-right"></i>&nbsp; '. lg('Restore from this Point'). '</a>';
					// jika autosave
...
?>		<!-- help -->
		...
		<?php
			...
			if(@$_GET['msg'] == 'published'){
				...
				echo '<div class="note note-success depth-xs"><i class="fa fa-check"></i> ' . lg('Post successfully restored.') . '</div>';
			}
...
?>								
								...
								<textarea name="content" cols="50" rows="20" class="form-control" id="text-editor" placeholder="<?php echo $lg_content?>"><?php echo $cp->content ?></textarea>
							  </div>
							</div> <!-- / .form-group -->
				...
				<div class="col-sm-3">
					<div class="panel-body" style="box-shadow: 2px 1px 5px rgba(0,0,0,0.1); margin-bottom: 2px;">
						...
						<div id="hidden_toggle">
							...
							<div id="aform" style="display: none">
								<select name="status" class="form-control">
									<option value="publish"<?php if($cp->status == 'publish'){ echo ' selected="selected"'; } ?>><?php echo lg('Published') ?></option>
						...
						<div id="hidden_toggle2">
							...
							<div id="aform" style="display: none">
								...
								<input id="child" type="text" class="form-control" name="post_password" style="display: none" placeholder="<?php echo lg('Post Password...') ?>"/>
								<div id="ok" class="btn btn-sm btn-default"><?php echo lg('Ok') ?></div>
								<a href="#" class="text-xs" id="c">&nbsp;<u><?php echo lg('Cancel') ?></u></a>
							</div>
						</div>
						<div id="hidden_toggle3">
							<span class="text-sm"><b><?php echo lg('Allow Commenting') ?>:</b>&nbsp;<a href="#"><u><?php echo $cp->comment_detail ?></u></a> </span>
								<div id="aform" style="display: none">
							...
							<div class="modal-dialog modal-sm">
								<div class="modal-content">
									...
									if(@$_GET['filter'] == 'deleted'){
										...
										echo '<h4 class="modal-title"><i class="fa fa-exclamation-circle"></i>&nbsp;&nbsp;'.lg('Delete Item').'</h4>';
									}
									?>
									</div>
									<div class="modal-body">
										...
										if(@$_GET['filter'] == 'deleted'){
											...
											echo lg('Are you sure to move this item to Recycle Bin?');
										}
										?>
										<div id="deltext"></div>
										<hr/>
